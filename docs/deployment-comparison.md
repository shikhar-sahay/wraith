...
| **Instance class** | `t3.micro` (Ubuntu 24.04, documented) | EC2 instance (Wraith deterministic, low overhead; see `infra/aws-setup.md`) | Partial - Mumbai class documented, US-East class not separately specified |
...
US-East Wraith captured substantially more post-auth command activity within its observed Wraith sessions (11 of 12 sessions, 21103 commands, 18 exact unique strings). However, raw count was highly skewed: 79.0% came from one repeated `echo -e "\x6F\x6B"` probe (`8.217.18.158` single session, see skewed analysis), and only 18 exact command strings were observed. The interesting finding is therefore not `21k commands`. The stronger finding is that several sessions (63, 119, 133, 371, 1877, 3709 commands) proceeded beyond authentication into repeated host/environment reconnaissance (`/bin/./uname`, `uptime -p`, `lspci`, `nvidia-smi`, `df -h`, `hostname`, `nproc`, `free -h`).
